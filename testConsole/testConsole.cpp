...
void test_load_dll_for_fallback_imd()
{
	HMODULE hm = ::LoadLibrary("IMDMagstripeFallback.dll");

	BOOL val = FALSE;

	if (NULL != hm)
	{
		cout << "CCardValidationState::IsFallbackIMD() - found IMDMagstripeFallback.dll";

		PFISFALLBACKIMD pfIsIMDFallback = (PFISFALLBACKIMD) ::GetProcAddress(hm, "IsFallbackIMD");

		LPCSTR szIMD = "123456";

		if (pfIsIMDFallback)
		{
			cout << "CCardValidationState::IsFallbackIMD() - checking IMD %s";
			cout << szIMD;
			val = pfIsIMDFallback(szIMD);
		}
